...
@section('title', ($type === 'shipping' ? 'お届け先' : '請求先') . '住所の編集')
    ...
    <div class="container">
        <div class="account-layout">
            ...
            <main class="address-card">
                <h1>{{ $type === 'shipping' ? 'お届け先' : '請求先' }}住所の編集</h1>
                ...
                <form action="{{ route('account.addresses.update', $address->user_address_id) }}" method="POST" class="address-form">
                    ...
                    <div class="address-group full">
                        ...
                        <input type="text" name="label" value="{{ old('label', $address->label) }}" placeholder="例: 自宅、会社">
                    ...
                    <div class="address-form-grid">
                        <div class="address-group">
                            ...
                            <input type="text" name="last_name" value="{{ old('last_name', $address->last_name) }}" placeholder="姓">
                            @error('last_name') <div class="error-text">{{ $message }}</div> @enderror
                        </div>

                        <div class="address-group">
                            <label>名<span>*</span></label>
                            <input type="text" name="first_name" value="{{ old('first_name', $address->first_name) }}" placeholder="名">
                    ...
                    <div class="address-group full">
                        ...
                        <input type="text" name="phone" value="{{ old('phone', $address->phone) }}" placeholder="電話番号">
                        @error('phone') <div class="error-text">{{ $message }}</div> @enderror
                    </div>

                    <div class="address-group full">
                        <label>会社名（任意）</label>
                        <input type="text" name="company_name" value="{{ old('company_name', $address->company_name) }}" placeholder="会社名">
                        @error('company_name') <div class="error-text">{{ $message }}</div> @enderror
                    </div>

                    <div class="address-group full">
                        <label>住所<span>*</span></label>
                        <input type="text" name="address" value="{{ old('address', $address->address) }}" placeholder="市区町村・番地">
                        @error('address') <div class="error-text">{{ $message }}</div> @enderror
                    </div>

                    <div class="address-group full">
                        <label>建物名・部屋番号（任意）</label>
                        <input type="text" name="apartment" value="{{ old('apartment', $address->apartment) }}" placeholder="建物名・部屋番号">
                    ...
                    <div class="address-group full">
                        ...
                        <select name="country">
                            ...
                            <option value="Japan" {{ old('country', $address->country) === 'Japan' ? 'selected' : '' }}>日本</option>
                            <option value="Thailand" {{ old('country', $address->country) === 'Thailand' ? 'selected' : '' }}>タイ</option>
                        </select>
                    ...
                    <div class="address-3-col">
                        <div class="address-group">
                            ...
                            <input type="text" name="city" id="city" value="{{ old('city', $address->city) }}" placeholder="市区町村">
