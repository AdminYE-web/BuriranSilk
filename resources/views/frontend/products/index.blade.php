...
@section(
    'meta_description',
    'タイシルクを使用した社員証ケース、キーホルダー、名刺入れなどの商品一覧です。'
)
            ...
            >
                ...
                <div class="product-filter-title">
                    <svg viewBox="0 0 24 24" aria-hidden="true">
                        <path
                            d="M4 6h3m4 0h9M9 4v4M4 12h9m4 0h3m-5-2v4M4 18h3m4 0h9M9 16v4"
                        />
                    </svg>
                ...
                <section class="product-filter-section is-open">
                    <button
                        class="product-filter-section-toggle"
                        type="button"
                        aria-expanded="true"
                    >
                        <span>カテゴリー</span>
                    ...
                    <div class="product-filter-section-content">
                        @foreach ($categories as $category)
                            <label class="product-filter-checkbox">
                                ...
                                <span
                                    class="product-filter-checkbox-box"
                                    aria-hidden="true"
                                ></span>
                ...
                <section class="product-filter-section is-open">
                    ...
                    <div
                        class="product-filter-section-content product-price-filter"
                    >
                        <div
                        ...
                        <div class="product-price-inputs">
                            ...
                            <span class="product-price-separator">
                                〜
                            </span>
                        ...
                        >
                            ...
                            @else
                                <div
                                    ...
                                    aria-label="{{ $product['name'] }} 準備中"
                                ></div>

                                <div
                                    class="product-card-placeholder-body"
                                    aria-hidden="true"
                                ></div>
                            @endif
                        </article>
...
@section('js')
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const inputMin =
                document.getElementById('priceInputMin');

            const inputMax =
                document.getElementById('priceInputMax');

            const slider =
                document.querySelector('.product-price-slider');

            const sliderRange =
                document.querySelector(
                    '.product-price-slider-range'
                );
            ...
            const clampPriceValues = function (source) {
                const minimum = Number(
                    slider.dataset.priceMin
                );

                const maximum = Number(
                    slider.dataset.priceMax
                );
                ...
                low = Math.max(
                    minimum,
                    Math.min(low || 0, maximum)
                );

                high = Math.max(
                    minimum,
                    Math.min(high || maximum, maximum)
                );

                if (low > high) {
                    if (
                        document.activeElement === rangeMin ||
                        document.activeElement === inputMin
                    ) {
                        low = high;
                    } else {
                        high = low;
                        ...
                        function () {
                            ...
                            const content =
                                section.querySelector(
                                    '.product-filter-section-content'
                                );
