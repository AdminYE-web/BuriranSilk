    ...
    <div class="product-detail-shell container">
        ...
        <section class="product-options-column" aria-labelledby="productDetailTitle">
            @php
                ...
                $editingArtworkNames = collect($editingCartItem['artworks'] ?? [])
                    ->mapWithKeys(fn ($artwork) => [
                        (string) ($artwork['group_id'] ?? '') => $artwork['original_name'] ?? '',
                    ])
                    ->all();
            @endphp
            <form
                id="productCustomizeForm"
            ...
            >
                            ...
                            @if ($displayType === 'previous_order_design')
                                <div class="product-option-stack">
                                    @foreach ($optionGroup['options'] as $optionIndex => $option)
                                        @php
                                            $normalizedOption = \Illuminate\Support\Str::lower(trim($option['code'] ?: $option['name']));
                                            $showsOrderNumber = in_array($normalizedOption, ['yes', 'はい', 'sim', 'true', '1'], true);
                                            $isChecked = $editingCartItem
                                                ? $editingSelectedOptionIds->contains((int) $option['id'])
                                                : ($option['is_default']
                                                    || (!$hasDefaultOption && $optionIndex === 0));
                                        @endphp
                                        <label class="product-option-line">
                                            <input
                                                ...
                                                @checked($isChecked)
                                                @required($optionGroup['is_required'])
                                            >
                                ...
                                <div
                                    class="product-option-stack product-repeat-order-details"
                                    data-previous-order-details
                                    hidden
                                >
                                    ...
                                    <input
                                        id="previousOrderNumber-{{ $optionGroup['id'] }}"
                                        type="text"
                            ...
                            @elseif ($displayType === 'font_option')
                                ...
                                <div class="product-option-stack">
                                    @foreach ($optionGroup['options'] as $optionIndex => $option)
                                        @php
                                            $normalizedOption = \Illuminate\Support\Str::lower(trim($option['code'] ?: $option['name']));
                                            $showsFontDetails = in_array($normalizedOption, ['yes', 'はい', 'sim', 'true', '1'], true);
                                            $isChecked = $editingCartItem
                                                ? $editingSelectedOptionIds->contains((int) $option['id'])
                                                : ($option['is_default']
                                                    || (!$hasDefaultOption && $optionIndex === 0));
                                ...
                                <div
                                    class="product-font-option-details"
                                    data-font-option-details
                                    data-font-group-id="{{ $optionGroup['id'] }}"
                                    hidden
                                >
                                    <div data-font-entries>
                                        <div class="product-font-entry" data-font-entry>
                                            <div class="product-font-entry-header">
                                                ...
                                                <button
                                                    type="button"
                                                    class="product-font-entry-remove"
                                            ...
                                            <div class="product-font-field">
                                                ...
                                                <div class="product-font-size-control">
                                                    <button type="button" data-font-size-action="decrease" aria-label="フォントサイズを小さくする">−</button>
                                                    <input
                                                        type="number"
                                                    ...
                                                    >
                                                    <button type="button" data-font-size-action="increase" aria-label="フォントサイズを大きくする">＋</button>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    ...
                                    <template data-font-entry-template>
                                        <div class="product-font-entry" data-font-entry>
                                            ...
                                            <label class="product-font-field">
                                                <span>名入れ文字・テキスト</span>
                                                <input type="text" placeholder="配置する文字をご入力ください" data-font-field="text">
                                            </label>

                                            <label class="product-font-field">
                                                <span>ご希望の書体・フォント</span>
                                                <input type="text" placeholder="例: ゴシック体 / 明朝体" data-font-field="font">
                                            </label>
                                            ...
                                            <div class="product-font-field">
                                                ...
                                                <div class="product-font-size-control">
                                                    <button type="button" data-font-size-action="decrease" aria-label="フォントサイズを小さくする">−</button>
                                                    <input type="number" min="1" max="200" value="12" data-font-field="size">
                                                    <button type="button" data-font-size-action="increase" aria-label="フォントサイズを大きくする">＋</button>
                                                </div>
                                            </div>
                                        </div>
                ...
                <fieldset class="product-option-group">
                    ...
                    <div id="repeatDesignDetails" class="product-option-stack product-repeat-order-details" hidden>
                        ...
                        <input
                            ...
                            type="text"
                            name="previous_order_number[legacy]"
                            autocomplete="off"
                            disabled
                        >
                    </div>
                ...
                <fieldset class="product-option-group">
                    ...
                    <label class="product-upload-field">
                        <span>データ入稿（印刷データをお持ちの方）</span>
                        <input id="artworkFile" type="file" name="artwork[legacy]" accept=".ai,.pdf,.eps,.psd,.png,.jpg,.jpeg">
                        <span class="product-upload-dropzone">
                            <svg viewBox="0 0 24 24" aria-hidden="true">
                                <path d="M7 3h7l5 5v13H7z" />
                                <path d="M14 3v5h5M12 17v-6m0 0-3 3m3-3 3 3" />
                 ...
                 <fieldset class="product-option-group">
                    ...
                    <label class="product-quantity-field">
                        <input
                            ...
                            name="quantity"
                            step="1"
                            value="{{ $editingCartItem['quantity'] ?? 100 }}"
                            required
        ...
        <div id="orderSummaryFlyout" class="product-order-flyout">
            ...
            <aside
                ...
                aria-labelledby="orderSummaryTitle"
            >
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            const getDependencyStates = function (selectedOptionIds) {
                ...
                return { groupStates: groupStates, optionStates: optionStates };
            };
            ...
            Object.entries(editingFontEntries).forEach(function ([groupId, savedEntries]) {
                ...
                while (entriesContainer.querySelectorAll('[data-font-entry]').length < savedEntries.length) {
